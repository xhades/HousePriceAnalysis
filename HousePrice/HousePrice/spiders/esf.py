...
class EsfSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # 翻页
        for i in range(1, 2):  # 共有100页 每页30条
    ...
    def parse_details(self, response):
        ...
        esf_item['zongjia'] = zongjia.strip()   # 总价
        esf_item['tax'] = tax.strip()   #
        esf_item['huxing'] = huxing.strip()  # 户型
        esf_item['mianji'] = mianji.strip()  # 面积
        esf_item['danjia'] = danjia.strip()  # 单价
        esf_item['chaoxiang'] = chaoxiang.strip()  # 朝向
        esf_item['louceng'] = louceng.strip()  # 楼层
        esf_item['zongcengshu'] = zongcengshu.strip()  # 总层数
        esf_item['zhuangxiu'] = zhuangxiu.strip()   # 装修
        esf_item['xiaoqu'] = xiaoqu.strip()   # 小区
        esf_item['quyu'] = quyu.strip()   # 区域
        esf_item['school'] = school.strip()  # 学校
